...
class CashbillServiceTestCase(unittest.TestCase):
    ...
    def test_issueResult(self):
        bulkSubmitID = "PYTHON-DJANGO-BULK"
        response = self.cashbillService.getBulkResult(self.testCorpNum, bulkSubmitID)

        result_list = response.__dict__["issueResult"]
        tradeDT_cnt = 0
        issueDT_cnt = 0
        for issue_result in result_list:
            tradeDT_cnt += 0 if hasattr(issue_result, "tradeDT") else 1
            issueDT_cnt += 1 if hasattr(issue_result, "issueDT") else 0

        self.assertEqual(len(result_list), tradeDT_cnt)
        self.assertEqual(len(result_list), issueDT_cnt)
