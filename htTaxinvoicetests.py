...
class HTTaxinvoiceServiceTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        self.htTaxinvoiceService = HTTaxinvoiceService(
    ...
    def test_updateContact(self):
        ...
        response = self.htTaxinvoiceService.updateContact(
            self.testCorpNum, contactInfo, self.testUserID
        )
        self.assertEqual(response.code, 1, "담당자 정보 수정성공.")

    def test_getCorpInfo(self):
        corpInfo = self.htTaxinvoiceService.getCorpInfo(
            self.testCorpNum, self.testUserID
        )
    ...
    def test_requestJob(self):
        ...
        SDate = "20190901"
        EDate = "20191231"
        jobID = self.htTaxinvoiceService.requestJob(
            self.testCorpNum, Type, DType, SDate, EDate, self.testUserID
        )
        print("작업아이디 : " + jobID)
    ...
    def test_getJobState(self):
        ...
        state = self.htTaxinvoiceService.getJobState(
    ...
    def test_listACtiveJob(self):
        jobInfos = self.htTaxinvoiceService.listActiveJob(
            self.testCorpNum, self.testUserID
        )
    ...
    def test_search(self):
        ...
        try:
            searchInfo = self.htTaxinvoiceService.search(
                self.testCorpNum,
                JobID,
                Type,
                TaxType,
    ...
    def test_summary(self):
        ...
        summaryInfo = self.htTaxinvoiceService.summary(
            self.testCorpNum,
            JobID,
            Type,
            TaxType,
    ...
    def test_getTaxinvoice(self):
        ...
        taxinvoiceInfo = self.htTaxinvoiceService.getTaxinvoice(
            self.testCorpNum, NTSConfirmNum, self.testUserID
        )
        self.assertIsNotNone(taxinvoiceInfo, "상세정보 조회")

    def test_getXML(self):
        NTSConfirmNum = "201607274100002900000209"
        taxinvoiceXML = self.htTaxinvoiceService.getXML(
            self.testCorpNum, NTSConfirmNum, self.testUserID
        )
        self.assertIsNotNone(taxinvoiceXML, "상세정보 조회-XML")

    def test_getFlatRatePopUpURL(self):
        url = self.htTaxinvoiceService.getCertificatePopUpURL(
            self.testCorpNum, self.testUserID
        )
    ...
    def test_getFlatRateState(self):
        flatRateInfo = self.htTaxinvoiceService.getFlatRateState(
            self.testCorpNum, self.testUserID
        )
